...
| 17 | `20260513002429_cost_rpcs.sql` | Cost-managing RPCs: `create_trip_full` / `replace_trip_full` now read the new cost fields + shares array, `replace_item_shares(item_id, jsonb)` atomically replaces an item's split rows, `set_trip_budget` is a one-call helper for the inline "Set budget target" CTA. |
| 18 | `20260513065319_pack_items_schema.sql` | Mobile redesign · Pack feature foundation: `trip_pack_items` (per-trip checklist of physical items) + `item_pack_items` (optional event-tagging link table that powers the mobile Today reminder). RLS inherits from the parent trip. |
| 19 | `20260513065438_cascade_defer_items_rpc.sql` | Mobile redesign · `cascade_defer_items(item_id, minutes)` RPC. Cascade-shifts an item plus all later same-day non-fixed items by N minutes (clamps at 23:59); returns the count of items that now overlap a fixed event so the client can surface a warning toast. |
